...
class GrundlagenController extends Controller
{
    public function show(Request $request): View
    {
        $module = collect(config('onboarding.modules', []))->firstWhere('key', 'grundlagen');
        ...
        $lessons = array_map(function (array $lesson) use ($completedSet, $module, $activeLesson) {
            $key = $lesson['key'] ?? null;
    ...
    public function toggle(Request $request, string $lessonKey): RedirectResponse
    {
        ...
        return redirect()
            ->route('modules.grundlagen', ['lesson' => $lessonKey])
            ->with('status', $message);
    }
